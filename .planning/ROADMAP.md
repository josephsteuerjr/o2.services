...
**Plans**: 5 plans, 5 waves (sequential — every plan updates `vitest.config.ts`'s hand-derived counts, and four of them plant, which this repository's shared working tree forbids doing in parallel)
- [ ] `33-01-PLAN.md` — the placement vocabulary: a closed jurisdiction union, a closed hint set, two separately-named helpers, one siting call
