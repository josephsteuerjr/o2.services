...
## Milestone v2.0 — Open the Doors (Phases 29-42)
...
- [x] **Phase 30: Inbound Listener Correctness & Hibernation** - More than five distinct clients a second are admitted, each with its own remote address, and backpressure has a defined answer instead of `undefined`
- [x] **Phase 31: Hosted Record Store, Its Expiry, and the Capability It Never Advertises** - The hosted node answers on `/o2/kad/1.0.0`, its records expire by an alarm-driven sweep landing in the same phase, and no record it publishes ever claims execution
...
Criteria 2 through 7 are met, three of them with plants watched red, and `29-REPORT.md` is the record. Criterion 2 is met in full: an outside peer dials, identify completes, and **one PeerId spans both construction boundaries** — a redeploy, and an eviction observed after an ≈8 h 50 m idle interval with no deploy in it. Re-read again today: `GET /self` answers `12D3KooWKm587fnGat5xncq9kaWUk4bN5gUJQiF4q8EwJnrb7rsz` at version `2.0.0-rc.10`, the same PeerId `29-EVIDENCE.md` recorded on 2026-08-28, so the identity has now survived five further days of eviction rather than the single interval that was measured.

**Criterion 1 is `Refuted` and permanently unsatisfiable, and it is the only thing holding the checkbox off.** It asks that the billing alert's configuration timestamp **precede** the deploy log that created the first Durable Object. It did not. No later alert makes that true, no wording repairs it, and `HOST-10` is the ledger's first `Refuted` row for exactly this. **The ordering can only be lost once per resource** — which is why it now stands at the top of `.planning/OWNER-ACTIONS.md` for the three objects Phase 33 would create and for the TURN key Phase 34 needs.

Closing uncounted rather than ticking is the honest form: the phase delivered its goal, one criterion is permanently false, and neither fact is allowed to erase the other. `completed_phases: 0` in `STATE.md` counts ticked checkboxes and is **correct**; it is not a measure of progress, and this block is why.
...
**AMENDED 2026-08-31 — CRITERION 1 IS NOW MET ON THE DEPLOYED NODE, and the reading is stronger than the criterion asks.** The box could not be ticked while `rc.5` was the deploy, because every reading was local. Against the deployed `v2.0.0-rc.7` object: **24 separate libp2p nodes dialled `/dns4/o2-bootstrap.af-4a0.workers.dev/tcp/443/tls/ws/p2p/12D3KooW...b7rsz` together and 24 of 24 were admitted in 987 ms** — 24 distinct connection ids, 24 distinct local peer ids, every one verifying the node's own PeerId through identify. **The anti-vacuity here is arithmetic rather than a plant, and it is stronger for it.** An earlier run of the same probe admitted 8 of 8 but took 1 705 ms, and that reading was DISCARDED as vacuous: the library default of 5 per second per host would have passed roughly eight connections over 1.7 s too, so it could not tell the shipped `HOSTED_INBOUND_THRESHOLD = 256` from the defect. Twenty-four inside one second cannot be produced at the default at all. **All 24 shared ONE source address**, because they ran on one machine — and that is the same shape the local reading had, where `wrangler dev` stamped `cf-connecting-ip: 127.0.0.1` on all eight; so the deployed run closes the criterion on identical terms, and what it additionally shows is that 24 admissions from a SINGLE host in under a second passed through the per-host limiter itself. **Criterion 3's deployed half stays OPEN and is not quietly folded in**: it asks for the remote address read back from *the node's own connection list*, and this tier serves no route that answers it. Adding one to close a criterion would invert the rule that every route is a surface. What was taken instead is the client-observable half the local e2e also takes — distinct connections, not collapsed onto one — and it is recorded as that and no more

**AMENDED 2026-08-31 (later the same day) — CRITERION 3'S DEPLOYED HALF IS MET, AND NO ROUTE WAS ADDED. THE PHASE CLOSES.** The paragraph above declined to add a route because *"adding one to close a criterion would invert the rule that every route is a surface"*, and that ruling stands — what changed is that the value was already on the wire. `@libp2p/identify` puts `connection.remoteAddr.bytes` into its response as `observedAddr` (`node_modules/@libp2p/identify/dist/src/identify.js:126`), gated on `IP_OR_DOMAIN.matches`, which was **run** against this tier's exact address form rather than read off a type: `/ip4/x/tcp/443/tls/ws` matches. So `observedAddr` **is** the entry in the node's own connection list, handed by the node to the peer it is about, over a protocol `createHostedLibp2p` has always registered. Nothing new is exposed and no test caused a capability to ship.

**The deployed reading.** A client dialled `v2.0.0-rc.7` and read the automatic identify: `observedAddr = /ip4/208.99.52.121/tcp/443/tls/ws/p2p/12D3KooWQ11Y6v2Yuk8HDAt7maKerEs3vuJx46nbBXqGpPpNV8tx`. **`208.99.52.121` was obtained INDEPENDENTLY** — from `https://o2-bootstrap.af-4a0.workers.dev/cdn-cgi/trace`, which Cloudflare's edge answers before the Worker, so the comparison introduces no third party and does not ask the node under test what the answer should be. The node reports the client's own public address: not the edge's, not a constant.

**And the reading can now go red, which it could not before.** The local e2e's criterion-3 case asserted distinct connection ids and distinct client peer ids, justified by *"the per-host inbound limiter would refuse from the sixth onward"* — **that justification was FALSE**, because this tier sets `HOSTED_INBOUND_THRESHOLD = 256` (NET-11, criterion 1's whole subject), so eight clients folded onto one address are admitted exactly as eight distinct ones are. Those two assertions could not fail on the failure the criterion names, and the file's own history says so: plant B, *"replacing the `CF-Connecting-IP`-derived address with one fixed address for every peer"*, had left it green. A case reading `observedAddr` was added, plant B was run again, and it goes **red and alone** — `expected '/ip4/203.0.113.9/tcp/443/tls/ws/p2p/…' to contain '/ip4/127.0.0.1/tcp/443/tls/ws'`, the other seven cases untouched. Restored `cmp`-clean. The false justification is corrected in place rather than deleted.

**No permanent test dials the deployed object.** The deployed reading is recorded here, as criterion 1's was: a spec that hit production on every run would bill the owner's account for a green.
...
**Status 2026-08-31 — ALL FIVE CRITERIA MET, and the phase closes.** The paragraph above stands as the reading of the day it was written; what changed is criterion 2, taken on the deployed object at `17:17:07Z` and recorded below with the control that made it a reading rather than a bare absence. `HOST-13` moved `Partial` → `Done` and left the re-read register by the ordinary exit.
...
**CRITERION 2 WAS ATTEMPTED ON THE DEPLOYED OBJECT ON 2026-08-31 AND FOUND A DEFECT — the reading the criterion asks for is what found it.** Against `v2.0.0-rc.7`: a peer joined `/o2/kad/1.0.0` with all four settings, dialled the deployed node, and called `provide()`. The write REACHED it — the event stream is `DIAL_PEER, SEND_QUERY, PEER_RESPONSE:12D3KooW...b7rsz, PATH_ENDED, FINAL_PEER, DIAL_PEER, SEND_QUERY, PEER_RESPONSE:12D3KooW...b7rsz`, both responses from the deployed node itself, with the publisher's routing table holding exactly that one peer. **A DIFFERENT peer then asked for the same CID and got nothing.** Run back to back in one process, both peers holding the deployed node in their routing tables: `findProviders` returned `[]`. **The deployed object accepts `ADD_PROVIDER` and does not serve the record back.**

**An earlier reading of this looked positive and was VACUOUS**, which is worth recording because it is the same trap as the `wrangler dev` persisted state: the publisher's own `findProviders` answered with the publisher's PeerId, because `provide()` stores locally as well as remotely and the local store is consulted first. A fresh peer that had never held the record is what turned the reading honest, and it answered `[]` thirty seconds later against the same CID.

**This is a defect on the hosted tier and NOT the same path `HOST-03` closed.** That requirement is about `/o2/<nodeKey>` VALUE records, measured writer-stopped-then-read on a local workerd. Provider records go through `@libp2p/kad-dht`'s separate providers store under a `/dht/provider/` prefix — which is one of the two namespaces `DoDatastore.put` refuses unless a sweep is armed. **WITHDRAWN WITHIN THE HOUR — THERE IS NO DEFECT, AND THE PROBE WAS THE DEFECT.** Everything above about the readings is accurate and the CONCLUSION drawn from them was wrong, which is the failure this project names as attributing by plausibility rather than by measurement. `@libp2p/kad-dht`'s `rpc/handlers/add-provider.js` **ignores a provider whose message carries no addresses** — `if (pi.multiaddrs.length < 1) { this.log('no valid addresses for provider %p. Ignore'); return }` — and answers the RPC anyway, which is why a `PEER_RESPONSE` arrived from a write that stored nothing. The probe peers were **dial-only**: no `addresses.listen`, therefore no multiaddrs to send, therefore silently dropped. Nothing on the hosted tier was involved.

**Ruled out by measurement, in order, before the cause was found** — recorded because each was a live hypothesis: `DoDatastore` refusing the `/dht/provider/` namespace (a swept store admits it, and `put`/`has`/`get` round-trip); the prefix query kad-dht loads providers with (`query({prefix})` returns the full key); and an in-memory cache in `providers.js` (there is none — it reads the datastore every time). What settled it was reading the LOCAL workerd's Durable Object storage directly through `--persist-to`: it held `/identity/seed`, `/journal/relay-service` and `/peers/...` and **no `/dht/provider/` key at all**, which moved the question from the read path to the write path.

**CRITERION 2's CLOCK IS NOW RUNNING ON THE DEPLOYED OBJECT — started 2026-08-31T16:01:02.294Z.** With a writer that listens on `/ip4/127.0.0.1/tcp/0/ws` and therefore has an address to send: CID `bafkreidss5xiinknhpb5p3kwdr2ft6jmka3pzon4f5x7ai3k2q5a2547oi`, provider `12D3KooWPYKqdBvMmWaRVoZysBN1daXUxvC2TdKpbZooAgBZ35t9`, and **present at T read back by a DIFFERENT, freshly-minted peer** — the reading that the first attempt got wrong. Both numbers come from `providerRecordPolicy()` and are not restated: validity `3_600_000` ⇒ gone by `2026-08-31T17:01:02Z`, sweep interval `900_000` ⇒ swept by `2026-08-31T17:16:02Z` at the latest. **The second half is a PENDING READING, not a closed one**: criterion 2 and `HOST-13` stay open until a fresh peer asks for that CID after `17:16:02Z` and gets nothing. The same arrangement is also the first genuine evidence for `HOST-13`'s platform half — whether Cloudflare fires the alarm on an instance that did not arm it — because a sweep that never runs leaves the record standing.

**READING TAKEN 2026-08-31T17:17:07Z — CRITERION 2 IS MET, AND THE FIRST ATTEMPT AT THE READING WAS VACUOUS.** A freshly-minted peer asked for that CID at T + 76 min and got `[]`. **That reading alone proves nothing**, and the control that says so was taken rather than reasoned: a provider record published *seconds earlier* also read back empty, so the absence was a property of the query and not of the record. The cause was in the probe — `dht.provide()` is an async generator and `await dht.provide(cid)` never runs it, publishing nothing. (Production is not affected: `dht-provider-announcer.ts:254` iterates it correctly.) With the publish actually performed, in **one run**: a **different, freshly-minted reader** — not the publisher, because `findProviders` consults the local provider store first — found the seconds-old record through the deployed relay and did **not** find the one from T. So the absence is a property of the record's age. `HOST-13`'s platform half is carried by the same reading and by one measurement already in the project record: `@libp2p/kad-dht@16.4.0` applies **no date filter** on the provider read path, so a record still in storage would have been returned. Its absence therefore means it was **deleted** — which on this tier is the alarm-driven sweep, fired by Cloudflare on an instance that did not arm it.
...
**THE RELAY ROLE IS NOW MEASURED ON THE DEPLOYED OBJECT — 2026-08-31, against `v2.0.0-rc.7`. This is the substrate criterion 1 stands on, and it is NOT criterion 1.** A Node peer carrying `circuitRelayTransport` and listening on `/p2p-circuit` dialled the deployed relay and **was granted a reservation**, resolving to `/dns4/o2-bootstrap.af-4a0.workers.dev/.../p2p-circuit/p2p/12D3KooWHUN2...5s99`. A second peer then reached it **only through that circuit** and verified its PeerId. `/self` moved across the three readings exactly as the shipped log says it should: `inboundHopStreams` 0 → 1 on the reservation → 2 on the dialler's CONNECT, `outboundStopStreams` 0 → 1 when the relay delivered, `relayService.bytes` 0 → 310 → 5 007, and `firstInboundHopStreamAt` set once at `1788191433180` and never moved. **`traffic.relayed` stayed exactly 0 throughout**, which is correct and is the stated granularity working rather than a gap: the relay's own legs are direct, and the payload it forwards is the third reading. So the relay-service log shipped in `rc.6` answered its first real question on the live node the day after it was written. **Criterion 1 remains OPEN**: it names two BROWSERS meeting over WebRTC with the relay leaving the data path, observable as the relay's byte counter flat while the pair's own moves — and this probe used Node peers with no WebRTC step. What it establishes is that the deployed relay grants reservations and routes CONNECTs, so a browser run that fails will be failing on the browser half.

**CRITERION 1 IS MET ON THE DEPLOYED RELAY — 2026-08-31, two real Chromium tabs.** Both loaded the demo from a Vite dev origin with `?relay=` naming only the deployed node, consented, and `autoStart`ed. Both reserved on it: `waitForWebrtcAddr` returned `/dns4/o2-bootstrap.af-4a0.workers.dev/.../p2p-circuit/webrtc/p2p/<B>`, an address that exists only because the deployed relay granted the slot. Tab A dialled it and `connectionsTo(B)` came back with **two** connections — `{limited: true, webrtc: false}`, the signalling circuit, and `{limited: false, webrtc: true}`, the direct pair — and `computePeers()` confirmed B answers over it.

**The criterion's own observable had to be corrected by measurement, and the correction makes it stronger.** It reads *the relay's byte counter flat while the pair's own counter moves*. The relay's counter is NOT flat and cannot be: `tab-api.ts` already records that an upgraded pair keeps its signalling circuit open beside the WebRTC connection, and yamux keeps that circuit alive, so the counter drips whether or not the pair is working. An absolute *flat* would therefore have failed on a fabric doing exactly the right thing. What separates *the relay is out of the data path* from *the relay is carrying this* is whether the drip CHANGES under load — a ratio taken inside one run, which is what `CLAUDE.md` § Measurement asks for anyway. Two windows of 12 s each, back to back on one pair:

| window | relay bytes | the pair's own bytes |
|---|---|---|
| idle | **+3 376** | — |
| busy, 38 `computePeers` rounds | **+2 728** | **+20 710** |

The busy window moved the relay LESS than the idle one. Thirty-eight rounds of work between two browsers added nothing the relay could distinguish from keep-alive, while the pair moved 20 710 bytes of its own. **The relay signalled and left the data path.** `inboundHopStreams` and `outboundStopStreams` were unchanged across both windows, which is the same statement from the other side.

**Criterion 2 is NOT closed and is the last one open here.** Its plant removes `addresses.announce` and watches reservations come back empty — on the DEPLOYED object that means deploying a knowingly broken configuration to the owner's production account and leaving a window in which the live relay hands every client an empty reservation. That is an owner decision, not an agent one.
...
**FIXED AND DEPLOYED 2026-08-31 — the finding above is closed, and it was reproduced before it was fixed.** The cause was not an oversight about which handler to register: the answer to `{kind:'reservations'}` existed in the tree exactly once, as a branch of `serveAgent`, and `AgentOptions` requires an `executor` and a `blockstore` with **no named opt-out** (`agent.ts:395-399`) — so a relay could not answer this question without also shipping a WASM executor it has no phase for. `serveReservations` (`packages/net/src/rendezvous.ts`) is that one branch alone, wired in `createHostedFabric` through `Libp2pTransport` + `RpcEndpoint` so the framing and the per-peer budgets stay one spelling on both ends, and refusing every other request kind **by name** rather than by silence. Reproduced first: `packages/cloudflare/src/hosted-rendezvous.e2e.test.ts` against the tree before the fix failed with *"the relay did not answer /o2/rpc/1.0.0 at all: expected +0 to be 1"*. Read on the **deployed** node after `v2.0.0-rc.7`: a seeker told only the relay's address got `answered: 1` and the reserver's `/p2p/<relay>/p2p-circuit/webrtc/p2p/<reserver>`, and an `offer` came back `this node serves reservations only, not offer`. `net/serveReservations` joins `OPEN_FINDINGS` — the first row on that register's standing condition that is **not** declared in `packages/cloudflare`, which widens the condition from *declared on the hosted tier* to *reached only through it*.
...
**CLOSING DECISION 2026-09-02 — criteria 1, 2 and 4 are MET, criterion 3's ordering is measured to be unanswerable, and `HOST-02` deliberately stays unchecked.**

**Criterion 3's darkness is now a reading rather than an absence.** The verdict rested on *the counters carry no history*; it now rests on the one surface that **does**. `relayService` is banked into Durable Object storage and refuses any write that would shorten it, so it is a lifetime total — and it has moved a long way, `inboundHopStreams` 2 → **96** and `bytes` 5 007 → **290 485** since 2026-08-31. But `firstInboundHopStreamAt` is `1788191433180`, **2026-08-31 15:50:33 UTC**, which falls roughly six hours after `rc.7` and three days after `rc.4`. The field records the first hop stream *since the journal existed*, never the first ever, so the `rc.4`→`rc.5` window — the only window the criterion is about — is outside anything this surface can see. **The verdict does not move: the ordering is unverified and may already be permanently false.** Part of the 96 is this project's own traffic, since the `rc.7` probe set that very timestamp; what is unattributed is the growth. Working: `32-EVIDENCE-ordering.md`.

**`HOST-02` stays unchecked on purpose, and this is the decision, not an oversight.** Its text is satisfied by what was observed on 2026-08-31 — two real Chromium tabs, a reservation that exists only because the deployed relay granted it, `connectionsTo(B)` returning both the limited circuit and the direct WebRTC pair, and a busy window that moved the relay **less** than the idle one while the pair moved 20 710 bytes of its own. Criterion 2 was met in a stronger form than it describes, against a local `workerd` with `ANNOUNCE_MULTIADDRS` emptied: the relay does not hand out empty reservations, it **refuses to build at all**, `Uncaught NoAnnouncedAddressError`, read by name from the runtime's own output.

**What is missing is not evidence but repeatability: neither run exists in the tree as a spec.** Both were one-off measurements. Ticking a ledger row on an unrepeatable reading is the widening of what counts as passing that this milestone has refused elsewhere, so the row waits for a committed arrangement — two browser contexts against a local `workerd` relay, with the deployed-relay run left standing as the separate record that the *deployed* object does it too.

**SUPERSEDED 2026-09-14 — the arrangement the paragraph above asks for is committed, and `HOST-02` is now `[x]`.** The paragraph is kept rather than rewritten because this tree retires a claim by dating it: it was true for twelve days and is the reason the row stayed open, which a reader needs in order to understand why the ledger carries two verdicts. `packages/cloudflare/src/two-tabs-hosted-relay.e2e.test.ts` is exactly what it specifies — two browser contexts against a local `workerd`, its own port and its own `--persist-to`, nothing deployed — and the deployed-relay run above still stands as the separate record. **The phase checkbox does not move**: criterion 3's ordering is measured to be permanently unanswerable, which is the closing decision recorded above and is unaffected by any of this. **What the new run changes about the reading itself, and it is a correction rather than a confirmation**: the table above records busy moving the relay LESS than idle (`+2 728` against `+3 376`) and treats that ordering as the finding. On the local object the order came out the OTHER WAY — idle `+6 498`, busy `+2 120` — so the ordering of two keep-alive drips is noise and an assertion on it would redden on a fabric behaving perfectly. The committed spec therefore asserts what noise cannot produce: the relay's `+2 120` against the pair's own `+140 580` in the same window, **1.5 %**. The conclusion the table draws is unchanged and now rests on a term that can fail.
...
**Status 2026-09-14 — all five plans executed and summarized, and each of the four success criteria splits cleanly into a built half and an owner-gated half.** `HOST-07`'s guard (33-04) refuses a listed term anywhere on the published surface and refuses a closed-set address named beside a placement verb anywhere in the tracked tree, two independent plants watched red — criterion 2 is MET. `HOST-06`'s placement vocabulary (33-01, 33-02) and criterion 1's negative proof (33-03) are built: two separately-named, non-branching helpers reach two different platform calls in a provable order, and a scratch compile watches the non-binding hint value refused where a binding value is required, with the platform's own runtime read as unable to carry that same distinction locally. `NET-15`'s drill and its weekly schedule (33-05) are built and green: three local stand-ins take an identical population through two arms of one run with the second stand-in terminated between them, the loss reported as a delta rather than against a threshold, and a guard reads the trigger set and the parsed cadence rather than trusting the workflow's own prose. **What holds every checkbox off is the same one act, named once rather than four times**: the phase's own scope fence forbids creating a Cloudflare resource, and criterion 1's objects, criterion 3's live reading and criterion 4's cross-region dialability all need the first `get()` against a real account — `.planning/OWNER-ACTIONS.md` row 2, unperformed. `HOST-06` and `NET-15` both stay `Partial` in `.planning/REQUIREMENTS.md` rather than `Done`, naming precisely this split; `HOST-07` alone reached `Done`, because its own row's claim — a document is refused before it can be written — needed no such act.
...
**Plans**: 1 plan — `.planning/phases/phase-34-two-fallback-rungs-below-direct-webrtc/34-01-PLAN.md`, covering criteria 1 and 2 only. Criterion 3 is CLOSED (`NET-13`, 2026-09-02) and criterion 4 is implemented with its residual recorded; neither is re-planned
**Status 2026-09-02 — criterion 3 is MET, criterion 4 is MET with a stated boundary, criteria 1 and 2 are planned and open.**

**Criterion 3 (`NET-13`) closed on a measurement, not an argument.** `RELAY_DATA_LIMIT_BYTES` read *"in total, in each direction"* — two readings at once, and a design takes the generous one. Its docblock now carries the correction and the primary source: `@libp2p/circuit-relay-v2`'s `createLimitedRelay` builds **one** `dataLimit` object and hands that same object to `countStreamBytes` for **each** direction. `relayedBudgetPerDirection()` is `limit / 2n`, **derived**, so the fabric holds one number that cannot drift. `packages/node/src/relayed-budget.node.test.ts` drives a real `circuitRelayServer()` on shipped defaults with two peers listening on nothing but `/p2p-circuit`: **62 KiB each way completes byte-identical, 63 KiB is cut**, and at 64 KiB the echo returns **49 152 bytes** — the identical figure the hosted relay returned for the same framing on 2026-08-24. Two unrelated relays, different processes, different transports, one accounting. The requirement's *"a test that exercises only direct WebRTC does not count"* clause is closed by a **positive control**: the identical exchange at the identical size over a direct connection completes.

**The counter is on wire bytes, so the ceiling is not reachable** — non-payload cost lands between 2 049 and 4 097 — and the constant says so rather than inviting a protocol to size at it.

**Criterion 4 is met, and the tick is recorded with the boundary rather than without it.** `Libp2pTransport.pathTo()` and `.controlOnlyPeers` classify a pair, and `send` refuses an over-budget frame on a control-only pair with a named `SendRefused` reason `'control-only-path'`, carried to `RpcFailure{kind:'send-refused', reason}`. The gate is **size AND path**, so control traffic still crosses — a path-only gate would silence discovery fabric-wide. **The boundary:** the refusal lands at whichever node would *send* the oversized frame, so a job whose bulk arrives as a **reply** is refused by name at the *serving* side while the requestor sees a timeout, and `pathTo` of a never-connected peer is `'unconnected'`, leaving first contact ungated. **And one assertion did not carry its claim, which is recorded in the file rather than hidden**: the refusal case's elapsed-time bound stayed green under its plant, because with the gate open the relay cut the circuit and the send failed in 173 ms rather than stalling. What the gate buys is **attribution, not speed** — `churn.ts` files `send-failed` under `'node'`, *"the peer never answered"*, so without it a requestor blames a peer that was never at fault.

**The provider is ruled and it was ruled on a probe: Cloudflare TURN.** The instruction was first *"use Google"*, and `tools/turn-provider-probe.mjs` refused it — Google's STUN hosts answer a Binding request and return **nothing at all** to an unauthenticated TURN Allocate, while `turn.cloudflare.com` answers **`0x0113` error 401**, which is a TURN server behaving as RFC 5766 specifies. **Cloudflare also answers on port 53**, which survives firewalls that drop 3478 — precisely the population this rung exists for. Working: `.planning/consults/2026-09-02-turn-provider-measured.md`.

**That consult also found a live defect this phase must fix. `browser-node.ts:1474` constructs `webRTC()` with no options, so `@libp2p/webrtc`'s four shipped defaults are the live ICE configuration and nothing in `packages/*/src/` sets `iceServers` at all — and one of the four, `stun.services.mozilla.com`, answers `NXDOMAIN` on `8.8.8.8`, `1.1.1.1` and `9.9.9.9`**, with `stun.cloudflare.com` resolving on the same resolvers as the control. Every tab spends a failing DNS lookup on every ICE gathering, on the exact path where `RUN-04`'s connection-classification stage is measured.

**Criteria 1 and 2 are planned, and the plan corrected a premise the brief carried.** `packages/cloudflare/` holds **no certificate check of any kind** — every hit for the enrolment symbols is in a `*.test.ts` fixture, zero in production source, and `hosted-capabilities.ts:36` states it outright: *"The hosted node holds no certificate."* So *"a request from outside the fabric is refused"* is **built in this phase**, not inherited, and it is the plan's largest item. A second correction is subtler and would have re-introduced the rot the phase removes: `getRtcConfiguration` does `config.iceServers ?? DEFAULT_ICE_SERVERS`, so a function form returning `{}` when a credential fetch fails hands every later gathering the four defaults again — `iceServers` is therefore made **non-optional in the return type**, so an omitting branch does not compile.

**Criterion 1 becomes a measurement rather than an owner act**, because `coturn` runs on loopback: a real RFC 5766 server means the pair, both expiry readings and the open-relay refusal are observed here. It closes **on mechanism** and explicitly does not carry the provider instantiation — nothing is observed against Cloudflare's own TURN until the owner creates a key.

**Criterion 2 does not close, and the reason is sharper than "Phase 33 has not run".** A cross-continent pair needs **clients on two continents** as well as three sited objects. Both are missing, `NET-12` stays unchecked, and no same-region substitute may be counted in its place.

**One cost accepted knowingly:** the ICE rot guard is the first e2e spec in this repository to depend on the public internet, so the e2e gate can now redden on a hiccup at a third party. Control-first ordering means an offline host is diagnosed rather than blamed on the list. That is the price of a guard that would actually have caught the Mozilla entry.

**Status 2026-09-02, criteria 1 and 2 — criterion 1 MET on mechanism, criterion 2 NOT closed.** Plan and summary: `.planning/phases/phase-34-two-fallback-rungs-below-direct-webrtc/34-01-PLAN.md` and `34-01-SUMMARY.md`.

**Criterion 1 has an observation for every clause.** A pair under `iceTransportPolicy: 'relay'` connects through a real **`coturn 4.17.2`** on loopback, read three ways from outside the page: the configuration actually handed to `RTCPeerConnection`, `typ relay` on the **selected** candidate pair via `getStats()`, and coturn's own log showing one allocation per tab's node key. Credentials arrive through `rtcConfiguration`'s **function form**. Expiry is refused twice over — time advanced, and time waited out. A request from outside the fabric is refused **by name**, with a valid-certificate positive control as its floor, and both halves are joined in one arrangement. **It does not carry the provider instantiation**: no Cloudflare key exists, nothing was observed against Cloudflare's own TURN, and that adapter is deliberately unwritten rather than written against documentation.

**Criterion 2 is built and not closed, and the missing evidence is two things, not one.** A cross-continent pair needs three *sited* objects (Phase 33) **and** clients on two continents. `NET-12` stays unchecked in both places with a `REREAD_REGISTER` entry carrying the promise; no same-region substitute was counted in its place.

**A finding that is this project's own worst failure mode, caught by measurement rather than by suspicion.** Without `--allow-loopback-peers`, coturn allocates, logs `ALLOCATE processed, success`, hands out a `typ relay` candidate — **and the pair silently never forms.** Every instrument green, nothing working. It is needed only for one-machine arrangements, which is exactly where it would be believed.

**Two plants stayed GREEN, and the first refuted an instruction in the plan itself.** A far-future credential left the expiry arm green, and the cause was isolated directly against coturn rather than attributed: `check_stun_auth: Cannot find credentials of user <4102444800:smoke>` — **coturn refuses a far-future credential identically to an expired one**, so the plant swapped one refusal for another. A modest `+3600 s` shift reddens it. The case was right and the plant was wrong; anyone reusing that plant gets a false green. The second: the rotation plant was green on the browser arm because its fresh contexts start with empty holders, so a broken cache has nothing to wrongly reuse — that arm proves *the server refuses an expired credential on a fresh gathering*, and rotation is carried by the unit case, which did redden at `expected 1 to be 2`. Neither case was weakened; the distinction is written into the spec.

**One task was refused rather than executed, and the refusal is the right outcome.** The plan asked to route the mint to a named region's Durable Object through `stubFor`. `worker.ts`'s `SERVED_BY` docblock records that as **the defect Phase 29 criterion 6 forbids** — a visitor's string would create and permanently site objects, and an object's location is fixed at its first `get()`. Sharding landed as a region tag inside the credential instead. **Phase 33's planning must resolve this**, since siting-aware routing is its subject.

**A correction the agent made to its own record**: the summary had quoted `7 493 ms` for one leg — a figure taken from a different arm in a different run, for a leg nobody had timed. Replaced with four same-run figures and that leg stated as **bounded rather than measured**, with the error left on the record instead of erased.

**Gates**, each read with `EXIT=$?` on the following line: `tsc --noEmit` **0**; `--project node` **0** (238 files / 3 406 tests); `--project browser` **0** (396 / 6 531, three engines); `--project e2e` **0** (61 / 311, banner *host was quiet*).
...
**Plans**: `.planning/phases/phase-35-conditions-of-entry-in-the-browser/35-01-PLAN.md` — 8 tasks, executed 2026-09-02, summary beside it
**Status 2026-09-02 — criteria 1, 2, 3 and 5 MET; criterion 4 is met in content and ordering and holds on one sentence that is the owner's.** Every proof is a reading rather than an internal flag. Criterion 1: a **network log** shows zero task-artifact requests with consent unrecorded — `fetchModuleForDispatch` takes a `GrantedConsent`, read at the gateway's own server and again through `page.on('request')`, with a positive control. Criterion 2: the indicator lives in **browser chrome**, not the page body, and was read from outside with a second page in front, in chromium, firefox and webkit — the page-body-only arm was watched failing in all three. Criterion 3 closed **both halves**: a multi-shard run does not complete across Stop, and `/self`'s `traffic.direct.connectionSeconds` stops accruing on a **local `workerd`**, so the socket the operator is billed for closes with the tab. Criterion 5: **11 000 bytes** disclosed against three real runs reporting 11 387 / 10 971 / 11 387, the literal hand-written so the guard does not compare a value with itself.
**The phase's most useful result is a plant that stayed GREEN, and it was reported rather than deleted.** Removing `worker.terminate()` from `stop()` alone left the case passing: the RPC is torn down separately, so through this instrument the termination is invisible. The claim is therefore carried by the wider plant — a `stop()` reduced to *stop accepting new tasks* — which reddened with *"the run held across Stop resolved with complete=true and 128 of 128 shards finished… the wait after Stop was 40498 ms, i.e. 3x"*. The green one is recorded in the summary and the case is not.
**Three corrections the tree forced.** `tasksExecuted` counts **admissions, not completions** — `GovernedExecutor` increments before calling `inner.execute`, so at full duty every shard read as executed before Stop and the guard's own precondition failed; the fixture runs at `dutyCycle: 0.5` and the finding is in the constant's docblock. `SETTLE_MS` of 2 000 was **shorter than the object's close latency** — the counter was still at 78.2 % of rate two seconds after Stop and settles at roughly 5 s; raised to 20 s **with the observed latency recorded**, not tuned until green. And a `required()` call sat in front of the fetch in `demo/main.ts`, which made criterion 1 structurally untestable because the page threw before it could refuse.
**`BROW-09` is `Partial` and the open leg is a ruling, not a symbol.** The disclosure states all four things in the required order — four plants, one per element, plus one for disclosure-after-click, all watched red; the one worth quoting turned the guarantee into a concession: *"«but» appears at character 33, before the guarantee at 61. A guarantee that arrives after a concession has been demoted to one."* What is missing is the sentence naming the telemetry's **legal basis**, which `.planning/REQUIREMENTS.md` § Open questions item 3 records as contested *across sources* and settled by legal review. Both readings are drafted verbatim so the ruling is a choice between two written sentences. `disclosure-four-elements.node.test.ts:212-231` currently asserts that `legitimate interest`, `legal basis` and `GDPR` appear **nowhere**, so that absence guard must be inverted in the same commit as whichever sentence lands.
**One caveat carried forward rather than buried:** the fetch gate lives in `fetchModuleForDispatch` and `BrowserNodeOptions` carries no consent field, so an embedding host that constructs a node directly and fetches by another path is not covered by this proof. Stated in the fixture's docblock and in the summary.
**Gates:** `tsc --noEmit` exit 0; `--project browser` exit 0 (360 files / 6045 tests); `--project e2e` exit 0 (49 files / 262 tests); `--project node` exit 0 (221 files, 3205 passed, 2 skipped).
...
**Plans**: `.planning/phases/phase-36-the-kill-switch-and-a-status-page/36-01-PLAN.md` — 10 tasks, executed 2026-09-02, summary and runbook beside it
**Status 2026-09-02 — all four criteria MET on local `workerd` readings, and the phase closed a live inversion nobody was looking for.**

**The defect it found first.** `AgentOptions.paused` was consulted only inside `serveAgent`, which answers **peers**. A tab's own work took `rpcAdmission`'s self-branch, whose port was a bare `LocalCapacity` that has never heard of `paused` — so **a paused tab refused every peer and went on computing its own work**, the exact inverse of the rule `browser-node.ts:2065-2071` states in its own words: *"a tab that admitted its own work by a rule other than the one it admits a peer's by would be more permissive to itself than to everybody else. One value, two readers, no way for them to disagree."* The comment described a mechanism that did not exist, and without closing it the phase's central observable was unobservable — a kill switch that stops only other people's work stops nothing. `BrowserNode.localAdmission` is now the one reading every local path takes, closing over the **same** `paused` binding rather than a second copy.

**Criterion 1**, three tabs on three local objects, one halted, one run, quiet host at `load/core 0.91`: `bootstrap-eu` → `took-no-work complete=false` with every shard `unagreed`, while `bootstrap-us` and `bootstrap-sam` → `took-the-work complete=true`. The plant reverting `admit:` to the pre-phase port reddened with *"the eu tab took a new run after its own region was halted… A halt a tab applies to its peers and not to itself stops nothing."* **Region slicing is enforced at the WRITE, not the read**: a `POST` to one object naming another's region is refused **409**, and under the global-switch plant that case went **200 ← 409**, which proves the region check rather than a body validation carries the refusal. **Criterion 3**: propagation **29 880 ms over 6 tabs at a 30 s poll**, ratio 0.996, two intervals compared inside one run.

**Three plants stayed GREEN, and the third is the phase's most important result.** (1) Deleting the `paused:` wiring left 58/58 unit tests green **and `tsc --noEmit` at exit 0** — `BrowserNodeOptions.paused` is optional, so there is no type-level backstop, and the whole claim that the switch reaches the node rests on one file written later in the same phase. (2) The client-side region plant changed nothing, because each tab polls only the object it dials. (3) **The plan's own instrument could not see the property, twice.** Sampling `tasksExecuted` put the halted tab **in the middle of the three** — `eu 0.681` against `us 0.489` and `sam 0.911`, on a quiet host with nothing broken — because admission is decided once at submit and the counter afterwards is a queue draining. The replacement also read green until a log showed **132 `admit` calls on the halted tab, none refused, and none from the probe at all**: CHURN-03's checkpoint was answering the repeated job without ever consulting admission. **Had the arithmetic fallen the other way, criterion 1 would have closed on a reading that meant nothing.**

**A defect introduced and repaired inside the phase, and it was a denial of service.** Refusing a write before reading its body left the request stream dangling — `Uncaught TypeError: Can't read from request stream after response has been sent` — and the next `GET /self` answered **500**. Two correctly-refused POSTs from a stranger could take a region's status offline. It was found only because that one spec pipes `workerd`'s stderr where every other spec discards it, which is a fact about this repository's test hygiene rather than about this phase.

**Criterion 4 was proved against the BUILT output, and that is how the production 404 was found.** `policy.html` had been linked from the live bundle and returning **404** on the deployed site since the consent gate shipped, because `vite.config.ts` declared no `rollupOptions.input` and only `index.html` was ever built. Both `status.html` and `policy.html` are now proved present in `dist/`. **A proof that reads the source tree cannot see a build that omits a file** — the same class as the two publish-path defects of 2026-08-28, both of which also failed silently.

**Open question 2 stays open, with the substitution disclosed rather than glossed**: the mechanism measured is a Durable-Object-storage poll, so Workers KV's ~60 s figure remains unmeasured. Criterion 3 asks only that the number exist, and it does. **No deployed reading**: every measurement is local `workerd`, and the deploy is an owner act in `36-RUNBOOK.md`.
...
**Plans**: `.planning/phases/phase-37-the-six-stage-funnel-and-a-frozen-telemetry-schema/37-01-PLAN.md` — 8 tasks, executed 2026-09-02, summary and runbook beside it
**Status 2026-09-02 — criteria 2, 3, 4 and 5 MET; criterion 1 met on local `workerd` with two caveats stated rather than buried. `RUN-04` Done, `RUN-05` Partial.**

The six stages move in order against a real relay and real WebRTC, read out of the Durable Object's persisted store **while the tabs were still running**. **Caveat one: there is no deployed reading** — the scope fence forbids the owner's account, so every number comes from local `workerd`, and turning this into a fabric reading is step 2 of `37-RUNBOOK.md`. **Caveat two: the first drop-off is structurally unmeasurable under the pending arming point.** `FUNNEL_ARMING` is `'at-consent'`, so a visitor who loads the page and never consents is never counted, and `stalledAt['page-load']` is always zero. Under reading A of open question 3 it stays zero forever; under reading B it becomes the most informative counter in the funnel.

**The shipped default is the INTERSECTION of the two readings, which is why nothing waited on the owner.** Everything reading A permits is also permitted under reading B, so nothing counted today becomes unlawful either way, and the ruling can only widen the population. The arming point gates every `enter()` through one flag, and the population string is written into the journal on every bank — so a later widening cannot retroactively relabel records already banked as `'opted-in-only'`, and the two populations stay distinguishable in the store.

**`RUN-05` is Partial and the reason is not a missing symbol.** The schema is frozen and digest-guarded, but the run that would test the freeze has not happened, and the row was registered in `REREAD_REGISTER` rather than closed by widening what counts as passing.

**Six plants stayed GREEN, and each exposed a blind instrument — this is the phase's most valuable output.** The worst: **the raw-IP plant did not redden because the store proof was not reading the store.** `node:sqlite` returns BLOBs as `Uint8Array` rather than `Buffer`, so `Buffer.from(String(value))` rendered the stored bytes as the text `255,15,66,…` — the scan was reading a decimal transcription of the envelope. With the decoder repaired the plant reddened naming both the key and the planted address. Two more from the same family: the identifier scan was **vacuous** (it scanned an empty string), then **falsely positive** on the legitimate 16-hex `schemaDigest` — fixed by excluding that field **by name**, never by widening the pattern, and given a floor asserting the scanned text is non-trivially long so a future empty read cannot pass. And the positive control tested the wrong thing: it supplied a country header and checked the header came back, which proves nothing about IP handling; rewritten to send a report with **no** country header and assert the answer is not `ZZ`.

**Corrections the tree forced.** `@libp2p/webrtc` captures `RTCPeerConnection` at module evaluation, so no statement inside `demo/main.ts` runs early enough to wrap it and the ICE observer saw nothing on a real dial — repaired with a side-effect import. Stage five counted the relay and so fired *before* stage four, inverting the roadmap's own order. `networkClass` could not ride in a request header because `sendBeacon` cannot set them, which would have systematically lost it on exactly the terminal reports that matter most. And **`T1`, Cloudflare's pseudo-country for Tor, contains a digit** and is refused by the letters-only rule: the test expectation was wrong and the code was right, so the expectation was fixed and **the loss recorded** — a Tor visit is now indistinguishable from one with no country header — rather than the rule widened to admit digits.

**A finding that belongs to the test harness rather than to this phase**, now recorded in `CLAUDE.md`: `wrangler dev` populates `request.cf` from the **host's real public address**, so every local run of a Cloudflare spec carries the operator's own city-level geolocation into the object under test. Nothing stores it — the hazard is the diagnostic somebody adds next to it.

**A schema limitation left open on purpose.** `stalledAt[k]` keys on the furthest stage entered, so *never tried to connect* and *tried and failed at the relay* land in the same bucket, and two of the six drop counters are structurally unreachable. Separating them means a second dimension in the record, which the frozen digest forbids until the freeze is deliberately broken — a later phase's decision, not a defect here.
...
**Goal**: As a volunteer whose device can be lost, seized or imaged, I want every long-lived secret this fabric writes to my device to be unreadable without my passphrase, so that losing the device does not also hand over the identity that speaks for me on the fabric.
**Mode:** mvp
...
**Depends on**: Phase 44 (the receipt already carries the issuers) — **done**, merged at `e5cf2a6` — and on an owner decision, **which was made on 2026-09-16: one provider.** The gate below is therefore settled and this phase is plannable. The ruling and what it costs are recorded at `.planning/OWNER-ACTIONS.md` §3c.
...
**THE GATE IS SETTLED — 2026-09-16, one provider, the lower ceiling accepted.** So the uncomfortable half is the half that lands: after this phase **no result this fabric produces is labelled `independent`**, because every member's certificate is signed by the same party. The strongest label becomes `single-issuer`. **The rule ships at full strength anyway** — `requireDistinctIssuers` defaults true and is not softened while one provider runs, because a rule relaxed until a second provider exists reports exactly the independence this phase exists to stop claiming. The day a second provider runs, `independent` becomes reachable with no code change. Two consequences to carry into the plan rather than discover: every fixture and figure in this tree that currently reads `independent` will move to `single-issuer` and must be read as a census of who was relying on single-issuer independence (Phase 44 found five such places), and **the public copy for the release must not promise an independence the fabric will not report** — that copy is Phase 39's, and this phase must check it rather than assume it.
